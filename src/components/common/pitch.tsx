...
const Pitch = ({
      ...
      if (players?.length) {
        formations.forEach((item, index) => {
          ...
          const basePositions = formationPositions.map((pos, idx) => {
            return { ...pos, player: players[index][idx] || null };
          });
  ...
  useEffect(() => {
    if (formations?.length) {
      loadPlayerInFormation(formations);
    }
  }, [formations, loadPlayerInFormation]);
  ...
  const renderPlayers = (positions: Position[], redirect = "left") => {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          width: formations.length > 1 ? "50%" : "100%",
        }}
      >
        {renderPlayers(homePositions)}
      </div>
      ...
      >
        {renderPlayers(awayPositions, "right")}
      </div>
    </div>
  );
};
