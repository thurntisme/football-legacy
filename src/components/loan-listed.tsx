...
import { Pencil, Share2, Star, Trash } from 'lucide-react';
...
type Props = {
  loanListedPlayers: Player[];
  handleRemoveFromList: (player: Player) => void;
};

const LoanListed = ({ loanListedPlayers, handleRemoveFromList }: Props) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {loanListedPlayers.length > 0 ? (
        loanListedPlayers.map((player) => (
        ...
        <div className="col-span-full text-center py-8">
          ...
          <h3 className="text-lg font-medium mb-1">No Loan Listings</h3>
          <p className="text-muted-foreground mb-4">
            You don't have any players on the loan list.
          </p>
        </div>
      )}
    </div>
